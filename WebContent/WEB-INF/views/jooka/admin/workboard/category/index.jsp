...
$.ajax({
	    type:"GET",
	    cache : false,
	    url:'${admin_root_path}/workboard/category/bytree',
	        	...
	        	$tree.on('tree.move', function(event) {
	        		if (confirm(event.move_info.moved_node.name + ' 을(를)\n\n' + event.move_info.target_node.name + '의 하위메뉴로 이동 하시겠습니까?')) {
	        			if(confirm(event.move_info.moved_node.name + ' 의 모든 하위메뉴들도 함께 이동됩니다.\n\n이동 하시겠습니까?')) {
	        				...
	        				get_item_info("${admin_root_path}/workboard/category/moveMenu", func, {"menu_idx":sel_id, "parent_menu_idx": parent_id}, $('body'));
	        			}
	        		}
	        		return false;
	        	});
	        	    ...
	        	    function(event) {
	        	        if (event.node) {
	        	            ...
	        	            if ((g_id.length < 1 || g_id != node.id) && node.id > 0)	{
		        	        	...
		        	        	$('.edit_submit').show();
		        	        	$('.insert_submit').hide();
	        	            	get_item_info("${admin_root_path}/workboard/category/getMenu", func, {"menu_idx" : node.id}, $('body'));
	        	            } else if (node.id == 0) {
		        	        	$('.insert_submit').show();
		        	        	$('.edit_submit').hide();
