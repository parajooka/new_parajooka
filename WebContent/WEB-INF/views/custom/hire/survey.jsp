	...
	<table class="survey_detail_table">
		...
		<c:if test="${hire.address_use == 0 || hire.address_use == 1}">
			<tr>
				...
				<td>
					<input type="text" name="post" id="post" readonly="readonly" onclick="sample6_execDaumPostcode()" placeholder="우편번호"> <input type="button" onclick="sample6_execDaumPostcode()" value="우편번호 찾기">
		...
		<c:forEach items="${questions}" var="question">
			<div class="question_box">
				...
				<c:if test="${question.type == 1}">
					<c:forEach items="${question.answers}" var="answer">
						<div class="answer_section">
							<textarea id="${question.id}_open_answer" name="${question.id}_open_answer" style="background-color: white; width: 100%; min-height: 200px;"></textarea>
						</div>
					</c:forEach>
				</c:if>
			</div>
