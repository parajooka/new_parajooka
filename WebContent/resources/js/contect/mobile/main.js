...
$(document).ready(function() {	
	...
	$('.complete').css('min-height', $(window).height());
	$('.nick-container').css('height', $(window).height());
	$('.popup-container').css('height', $(window).height());
	...
	$('.point_btn').on('mouseenter', function(){ 
			...
			var func = function(data) {
				...
				var count_timer = setInterval(function() {
					...
					if (timer >= 3) {
						...
						$('.point-section').css('background', 'none').css('color', 'black');
	...
	$('.show_complete').on('click', function() {
		if (!$('.complete_message_box').is(":animated")) {
			var func = function(data) {
				...
				$('html, body').css('height', ($('.complete').height() + $(window).height()/2) + "px");	
			...
			get_item_info("/contect/complete", func, {'nickname':$('.field-long').val(), 'level' : level, 'difficulty' : difficulty},  $('body'));
		}
	});
